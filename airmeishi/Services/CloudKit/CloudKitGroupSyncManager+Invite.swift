...
extension CloudKitGroupSyncManager {
  ...
  func createInviteLink(for group: GroupModel) async throws -> String {
    if group.isPrivate {
      ...
      guard let url = share.url else {
        throw NSError(
          domain: "GroupError",
          code: 500,
          userInfo: [NSLocalizedDescriptionKey: "Share created but no URL available. Try again."]
        )
      }
    ...
    let predicate = NSPredicate(
      format: "targetGroup == %@ AND createdBy == %@ AND isActive == 1",
    ...
    let sortedResults =
      results.compactMap { result -> (CKRecord, Date)? in
