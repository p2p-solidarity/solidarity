...
final class KeychainService {
    ...
    private func generateSigningKey(useSecureEnclave: Bool) -> CardResult<Void> {
        ...
        let isDuplicateError = errorCode == -25293 || errorCode == -25299 || 
                               message.contains("-25293") || message.contains("-25299") || 
                               message.contains("duplicate") || message.contains("errSecDuplicateItem")
        
        if isDuplicateError {
            print("[KeychainService] Duplicate error detected (code: \(errorCode)), switching to session-based non-persistent key...")
            ...
            return .failure(.keyManagementError("Failed to generate device key (persistent and session methods failed): \(message); session error: \(sessionMessage)"))
        }
        ...
        if status == errSecItemNotFound {
            ...
            case .success:
                ...
                guard retryStatus == errSecSuccess else {
                    return .failure(.keyManagementError("Failed to retrieve signing key after creation: \(statusDescription(retryStatus))"))
                }
                guard let candidate = item else {
