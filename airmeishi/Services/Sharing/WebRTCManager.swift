...
class WebRTCManager: NSObject, ObservableObject {
  ...
  func handleSignalingMessage(_ message: SignalingMessage, from peerID: MCPeerID) {
    ...
    guard let currentRemote = remotePeerID, currentRemote == peerID else {
      print(
        "[WebRTC] Ignoring signaling message from \(peerID.displayName) (current remote: \(remotePeerID?.displayName ?? "nil"))"
      )
      return
    }
    guard let pc = peerConnection else {
    ...
    case .candidate(let iceCandidate):
      print(
        "[WebRTC] Received ICE candidate (sdpMid: \(iceCandidate.sdpMid ?? "nil"), sdpMLineIndex: \(iceCandidate.sdpMLineIndex))"
      )
...
extension WebRTCManager: RTCPeerConnectionDelegate {
  ...
  func peerConnection(_ peerConnection: RTCPeerConnection, didGenerate candidate: RTCIceCandidate) {
    print(
      "[WebRTC] ICE candidate generated (sdpMid: \(candidate.sdpMid ?? "nil"), sdpMLineIndex: \(candidate.sdpMLineIndex))"
    )
