...
extension IDView {
  ...
  internal func didCapsule(title: String, subtitle: String, isActive: Bool, action: @escaping () -> Void) -> some View {
  ...
  internal var isDidKeyActive: Bool {
    guard let did = profile.activeDID?.did else { return true }
    return did.hasPrefix("did:key")
  }

  internal func switchDID(_ method: DIDService.DIDMethod) {
    coordinator.switchDID(method: method)
  }

  internal func shortDid(_ did: String) -> String {
  ...
  internal var coreSection: some View {
    RippleButton(
      state: rippleState,
  ...
  internal func handleCoreLongPress() {
  ...
  internal func groupRow(group: GroupModel) -> some View {
    Button(action: {
      selectedGroup = group
      presentProof()
    }) {
