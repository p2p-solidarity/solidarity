...
- [x] **S8h** badge 驗證存 state、停止每次 focus 重驗(用戶要求 2026-07-17:15 分鐘 TTL 內信任已存結果(`shouldReverifyBadge`),不再每次 Me tab focus 開 3 個 relay WS;誠實護欄:evidence sheet 顯示「上次檢查時間」、claim 不符即棄、record 重簽(`saveProfile`)與 publish 成功都會 `invalidateCachedNostrResult` → 有更動就清舊資料、badge 重驗成 stale 提示用戶重新 publish;onboarding 驗完也寫入 cache 供 Me tab 直接沿用)
- [x] **S8g** T7 signing-key 同步 race 修復(grill 定案 2026-07-17:雙鑄真因 = 兩把 syncable key 是不同 keychain item(primary key 含公鑰 hash)→ 兩把都同步 → 同類別內 MatchLimitOne 不確定 → DID 翻轉。三層修:① native `copyECPrivateKey` 同類別 MatchLimitAll + 公鑰 hash 升冪確定性選擇(跨裝置可證明收斂;不賭 creationDate 同步語義)、`generateSyncableP256Key` 只清非同步 class + 鑄前最後一刻改採已到達的 synced key、新 `listSyncableP256Keys`/`deleteSyncableP256Key`(nitrogen 0.35.7 regen;Android 誠實空表);② onboarding SecureKeysStep:`restoredFromICloud` 且無 key → 有上限等待(6×2.5s,CloudSyncPulse 動畫)後仍保證 ensureSigningKey(key 存在是硬保證 — 用戶定案);③ settings/dids 衝突解決區(>1 把才顯示,用戶顯式選保留、Face ID 'delete' always-prompt gate,絕不自動刪)。剩餘:真機雙裝置驗證(native 行為無法在 bun 驗)、mnemonic-only 回歸用戶僅靠收斂層保護。**-5 回歸修復(2026-07-18)**:S8g 的列舉式解析(MatchLimitAll+attributes)套到非同步 SE 類別觸發 passport proof CryptoTokenKit -5 回歸 —— 修正為列舉解析只用於 synchronizable=true,SE/legacy 類別還原 c3c38c8 原查詢(MatchLimitOne+ref);codex 實作、Claude review)
- [x] **S8f** 發布可靠性 + 部分成功可接受(2026-07-17 實測診斷:`relay.nostr.band` 整站 TCP 不通 → 換 `relay.primal.net`;damus 首連 ~25% 快速 WS error → `PublishResult.failure` 判別 + transport-only 重試 2 次(policy 拒絕與 timeout 永不重試);per-relay 拒絕訊息進錯誤 trace 與 Details;**用戶拍板:每份副本 ≥1 relay 接受 = partial 可過**(`isNostrPublishOutcomePartiallyAccepted` — 頁面副本與 kind-0 驗證副本各需 ≥1,驗證者只要一個 relay 同時有兩者即可解析;任一副本 0 接受仍為 error)— outcome step 誠實顯示 partial 面板、onboarding 不再被 sub-quorum 卡死;雙 quorum 仍是 published 完整成功門檻)
