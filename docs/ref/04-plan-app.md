...
- [x] **S8g** T7 signing-key 同步 race 修復(grill 定案 2026-07-17:雙鑄真因 = 兩把 syncable key 是不同 keychain item(primary key 含公鑰 hash)→ 兩把都同步 → 同類別內 MatchLimitOne 不確定 → DID 翻轉。三層修:① native `copyECPrivateKey` 同類別 MatchLimitAll + 公鑰 hash 升冪確定性選擇(跨裝置可證明收斂;不賭 creationDate 同步語義)、`generateSyncableP256Key` 只清非同步 class + 鑄前最後一刻改採已到達的 synced key、新 `listSyncableP256Keys`/`deleteSyncableP256Key`(nitrogen 0.35.7 regen;Android 誠實空表);② onboarding SecureKeysStep:`restoredFromICloud` 且無 key → 有上限等待(6×2.5s,CloudSyncPulse 動畫)後仍保證 ensureSigningKey(key 存在是硬保證 — 用戶定案);③ settings/dids 衝突解決區(>1 把才顯示,用戶顯式選保留、Face ID 'delete' always-prompt gate,絕不自動刪)。剩餘:真機雙裝置驗證(native 行為無法在 bun 驗)、mnemonic-only 回歸用戶僅靠收斂層保護)
