...
class CardManager: BusinessCardManagerProtocol, ObservableObject {
  ...
  func createCard(_ card: BusinessCard) -> CardResult<BusinessCard> {
    ...
    if businessCards.contains(where: {
      $0.name.lowercased() == cardForStorage.name.lowercased() && $0.id != cardForStorage.id
    }) {
  ...
  func searchCards(query: String) -> CardResult<[BusinessCard]> {
    ...
    let filteredCards = businessCards.filter { card in
      card.name.lowercased().contains(lowercaseQuery) || card.title?.lowercased().contains(lowercaseQuery) == true
  ...
  func getCardsByCategory(_ category: String) -> CardResult<[BusinessCard]> {
    let filteredCards = businessCards.filter { card in
      card.categories.contains(category) || card.skills.contains { $0.category == category }
    }
    return .success(filteredCards)
  }
  ...
  func getStatistics() -> CardStatistics {
    ...
    let averageSkillsPerCard =
      totalCards > 0 ? Double(businessCards.reduce(0) { $0 + $1.skills.count }) / Double(totalCards) : 0
