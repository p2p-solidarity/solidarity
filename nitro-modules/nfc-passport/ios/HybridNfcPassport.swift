...
final class HybridNfcPassport: HybridNfcPassportSpec {
  ...
  // MARK: - Trust Resources

  func getRevocationSnapshotJson() throws -> String {
    guard let url = Self.locateRevocationSnapshot() else {
      throw self.error(
        code: "revocation_snapshot_missing",
        message: "passportRevocationSnapshot.v3.json is not bundled."
      )
    }
    ...
    do {
      ...
      throw self.error(
        code: "revocation_snapshot_read_failed",
        message: "Failed to read passportRevocationSnapshot.v3.json: \(error.localizedDescription)"
      )
    }
  }
  ...
  /// Resolve the OpenAC v3 revocation snapshot generated by
  /// `scripts/generate_masterlist.py --revocation-source ...`.
  /// Missing means the caller must fail closed; we never synthesize an empty
  /// revocation set at runtime.
  internal static func locateRevocationSnapshot() -> URL? {
    let resource = "passportRevocationSnapshot.v3"
    let moduleBundle = Bundle(for: HybridNfcPassport.self)
    if let url = moduleBundle.url(forResource: resource, withExtension: "json") {
      return url
    }
  ...
  // MARK: - Mapping NFCPassportModel → PassportReadResult
    ...
    /// DG15 Active Authentication evidence for the OpenAC v3 circuit, or nil.
    ///
    /// Emitted ONLY for ECDSA-P256 AA keys — the passport_adapter circuit
    /// verifies `ecdsa_secp256r1(aa_pk, aa_signature, aa_challenge[32])`, so
    /// RSA-AA passports (ISO 9796-2 signatures) cannot satisfy it and fall
    /// through to the `requireAA` fail-closed path.
    ///
    /// The chip signs `digest(RND.IFD)`; the OpenAC circuit's `aa_challenge`
    /// is that 32-byte digest. NFCPassportReader returns the raw `r ‖ s`
    /// ECDSA signature in `activeAuthenticationSignature`. We assume the
    /// ICAO-default SHA-256 AA digest (the only width that fits the 32-byte
    /// `aa_challenge`); the Rust witness builder re-verifies the triple with
    /// the real DG15 key and fails closed if the chip used another digest.
    private static func activeAuthEvidence(model: NFCPassportModel) -> String? {
      guard model.activeAuthenticationSupported,
            model.activeAuthenticationPassed,
            let dg15 = model.getDataGroup(.DG15) as? DataGroup15,
            dg15.ecdsaPublicKey != nil
      else { return nil }

      let signature = model.activeAuthenticationSignature
      let challenge = model.activeAuthenticationChallenge
      // Raw P-256 ECDSA is r‖s = 64 bytes; anything else is not the layout
      // the circuit's `[u8; 64]` signature input expects.
      guard signature.count == 64, !challenge.isEmpty else { return nil }

      let aaChallengeDigest = Data(SHA256.hash(data: Data(challenge)))
      let payload: [String: String] = [
        "challengeB64": aaChallengeDigest.base64EncodedString(),
        "signatureRawB64": Data(signature).base64EncodedString(),
      ]
