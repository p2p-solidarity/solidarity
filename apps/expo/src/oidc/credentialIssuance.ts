...
/**
 * Parse an `openid-credential-offer://` URL or a raw credential_offer JSON
 * string. Mirrors Swift `parseOffer`/`decodeOffer`.
 */
...
/** Async variant — fetches `credential_offer_uri` when needed. */
...
async function executeCredentialRequest(opts: {
  ...
  if (!response.ok) {
    let json: RawCredentialResponse | null = null;
    try {
