# Solidarity (airmeishi) — Repo Guide for Agents

**The app is `apps/expo/` (Expo Router + React Native + TypeScript).** The original SwiftUI app was deleted on 2026-06-27 (commit `3fd308e`). Any doc or comment referencing Swift files (`solidarity/Views/...`, `Color.Theme.*`, `Themed*ButtonStyle`, `solidarity.xcodeproj`) is historical naming — do NOT hunt for those files; the live equivalents are listed below.

## Where truth lives (read in this order)

1. `apps/expo/CLAUDE.md` — the coding rulebook. Mandatory before editing anything under `apps/expo/`.
2. `docs/ref/06-plan-convergence-2.0.0.md` — the live **batch** plan (2.0.0 north star, Q1–Q6 decisions, old-feature keep/cut, execution plan), `docs/ref/07-plan-web-standalone.md` — the web-standalone phases 1–3, and `docs/ref/08-plan-passkey-root-sync.md` — the live **zero-friction App↔Web root sync** plan (WebAuthn PRF unlock, opaque ciphertext-only service, web key UI removed; supersedes 07's local-only web vault UX). `docs/ref/09-plan-pro-entitlement.md` — Pro subscription (IAP via expo-iap, entitlement rules, store-config checklist, unbuilt license bridge). `.superpowers/sdd/progress.md` — review ledger when present (records real security bugs already caught — read before touching security code).
3. `docs/ref/01–05` — spec reference, not plans: 01+03 = 1.3.3 "Verified Page" product spec (§6 of 03 is the kept/evolved/frozen/deleted table); 05 = QR/exchange wire SSOT (06 defers to it). 04 is the superseded 1.3.3 plan — history only.
4. **STALE — do not trust**: `README.md`, `spec.md`, `architecture.png` describe the deleted SwiftUI app and pre-pivot architecture (MultipeerConnectivity and CloudKit group sync are deleted; Semaphore/ZK groups frozen). Deleted pre-2.0.0 plans/specs are distilled in `docs/ref/notes-archive-pre-2.0.0.md`.

## Workflow — every coding task

1. Invoke the `flow` skill first: size the task S/M/L and follow its process. UI work also reads `.claude/skills/ui-polish/SKILL.md`.
2. Verify before claiming done (from repo root): `bun run typecheck && bun run lint && bun run test` (unit + parity).
3. Monorepo (bun workspaces): `apps/expo` = the app; `packages/shared` = pure-TS crypto/types consumed everywhere; `packages/parity-fixtures` = **frozen** golden vectors (the Swift source that generated them is deleted — never edit or regenerate); `nitro-modules/*` = native bridges (Swift/Kotlin behind TS specs).

## Security — hard rules (current TS terms)

- Sensitive paths: `apps/expo/src/{passport,keychain,identity,vault}` and `nitro-modules/{attest,keystone}` (attest = mrz-ocr+nfc-passport+passport-zk+semaphore merged; keystone = secrets-vault+spruce-did+cloudkit merged). Any change here is size **L** in flow — never S.
- Never read, print, or commit: `apps/expo/secrets/`, any `.env*`, `apps/expo/infra/terraform/terraform.tfstate*` / `terraform.tfvars`.
- Security-path errors use tagged unions (e.g. `BiometricResult`) — never throw raw, never swallow, never log PII.
- Face ID gates: `ALWAYS_PROMPT_ACTIONS` (rotateMasterKey, revealRecoveryBundle, deleteZKIdentity) deliberately bypass the biometric grace window (`apps/expo/src/keychain/sensitiveActionPolicy.ts`). Adding a grace period there is a security regression, not an optimization.
- Nitro `ArrayBuffer` args are non-owning: touching `buffer.data`/`.size` inside `Promise.async` is an uncatchable native crash. Copy the bytes synchronously first. Treat every new HybridObject method with an ArrayBuffer param as suspect.
- ZK honesty: the Android libc++ ABI bug can downgrade real ZK proofs to SD-JWT fallback (`trustLevel: 'white'`, see `nitro-modules/attest/KNOWN_ISSUES.md`). Never present a fallback as a real ZK attestation.
- Threat-model defaults (each from a real caught bug in `progress.md`): P2P transports scoped per-connection not per-topic; VC/VP must verify holder binding; OAuth discovery is https-only with host validation; no manual pre-hash before ES256 signing (the lib hashes).

## Traps (verified 2026-07-05)

- `.claude/worktrees/agent-*` are git worktrees frozen at pre-deletion commits — they still contain the old Swift tree. **Exclude them from every search.**
- Root `ios/`, `android/`, `app.json`, `solidarity.xcodeproj/`, `ci_scripts/` are stale artifacts. Real native projects: `apps/expo/{ios,android}` (regenerated by prebuild). Real CI script source: `apps/expo/ci-scripts/ci_post_clone.sh`.
- `packages/parity-fixtures/` is frozen: the Swift `FixtureExporter` that generated it was deleted — never regenerate or edit fixtures (its README explains the policy).
- No fake data — ever: data-driven UI renders exactly `loading` / `ready` / `error`; never a plausible placeholder. Full rule with examples: `apps/expo/CLAUDE.md` Rule 8.

## Build

iOS 26 / Android minSdk 29. Bundle id `kidneyweakx.airmeishi`. iOS releases via Xcode Cloud; `-skipPackagePluginValidation` is required by SPM plugin validation (explanation in `apps/expo/ci-scripts/ci_post_clone.sh`).
